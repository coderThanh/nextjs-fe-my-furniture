...
export type HeaderMenuDropDownProps = {
  title: ReactNode;
  children: ReactNode;
  isOpen: boolean;
...
export type HeaderMenuDropDownOnClick = (nextStatus: boolean) => void;

export default function HeaderMenuDropDown(props: HeaderMenuDropDownProps) {
  const relWrap = useRef<HTMLDivElement>(null); // nho them null

  const [isOpen, setIsOpen] = useState(false);

  const [offsetLeftWrap, setOffsetLeftWrap] = useState<number>();
  ...
  function onClickTitle(isNextStatus: boolean) {
  ...
  function getPostionX() {
    ...
    const wrapX: number | undefined =
      relWrap.current?.getBoundingClientRect().x;
